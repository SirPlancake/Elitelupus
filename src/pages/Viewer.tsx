...
import {Dices, Pause, Play, RefreshCw, Upload} from "lucide-react";
...
function ViewerLoadingScreen({State}: {State: ViewerLoadState}) {
    ...
    const Detail = State.Detail || "Preparing assets";

    return (
        <div className="absolute inset-0 z-40 flex flex-1 items-center justify-center">
            <div className="flex w-full max-w-sm flex-col items-center px-6 text-center">
                ...
                <p className="mt-6 max-w-full truncate text-base font-medium text-zinc-100">{State.Label}</p>
                <p className="mt-1 max-w-full truncate text-xs text-zinc-500">{Detail}</p>

                <div className="mt-4 flex w-full max-w-56 items-center gap-3">
                    <div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-zinc-800">
                        <div className="h-full rounded-full bg-lime-400 transition-[width] duration-300 ease-out" style={{width: `${Progress}%`}} />
                    </div>
...
export default function Page() {
    ...
    const SelectSkin = useCallback((Skin: SkinObject) => {
    ...
    }, [CustomFile, SelectedSkin?.image_url, setSearchParams]);
    ...
    const HandleSkinChange = useCallback((Value: unknown) => {
        ...
        SelectSkin(Skin);
    }, [CustomFile, CustomSkinName, SelectSkin, Skins]);

    const HandleRandomSkin = useCallback(() => {
        if (!Skins.length) return;

        const AvailableSkins = Skins.filter((Skin) => CustomFile || SelectedSkin?.image_url !== Skin.image_url);
        const SkinPool = AvailableSkins.length ? AvailableSkins : Skins;
        const RandomSkin = SkinPool[Math.floor(Math.random() * SkinPool.length)];

        if (!RandomSkin) return;

        SelectSkin(RandomSkin);
    }, [CustomFile, SelectedSkin?.image_url, SelectSkin, Skins]);
    ...
    const TopbarContent = useMemo(() => {
        ...
        return (
            <>
                ...
                <button aria-label="Randomize skin" title="Randomize skin" disabled={!Skins.length} onClick={HandleRandomSkin} className="flex h-10.5 items-center justify-center rounded-md border border-zinc-700 bg-zinc-800 px-3 py-1 text-white transition hover:cursor-pointer hover:border-zinc-600 hover:bg-zinc-700 disabled:pointer-events-none disabled:opacity-40">
    ...
    }, [DisplaySkinName, DisplaySkins, HandleModelChange, HandleRandomSkin, HandleSkinChange, HandleUpload, Loading, Models, Rotating, SelectedModel?.name, Skins.length]);
