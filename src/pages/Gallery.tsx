...
import {ChevronLeft, ChevronRight, Heart, RefreshCw, Search as SearchIcon, X} from "lucide-react";
...
type SkinFilterValue = "all" | "new" | "favourited" | `${number}`;
...
const FavouriteSkinsStorageKey = "elitelupus:favourite-skins";
...
const GetSkinFavouriteKey = (Skin: SkinObject) => String(Skin.id);

const ReadFavouriteSkinIds = () => {
    try {
        const StoredValue = window.localStorage.getItem(FavouriteSkinsStorageKey);
        const ParsedValue = StoredValue ? JSON.parse(StoredValue) : [];

        return new Set<string>(Array.isArray(ParsedValue) ? ParsedValue.map(String) : []);
    } catch {
        return new Set<string>();
    };
};

const WriteFavouriteSkinIds = (FavouriteSkinIds: Set<string>) => {
    try {
        window.localStorage.setItem(FavouriteSkinsStorageKey, JSON.stringify([...FavouriteSkinIds]));
    } catch {
        return;
    };
};

function SkinPreviewImage({src, alt}: {src: string; alt: string}) {
    ...
    return (
        <div className="relative h-23 w-2/4 shrink-0 overflow-hidden rounded-md border border-zinc-700/75 bg-zinc-900/75 shadow-inner">
            {!Loaded && (
...
export default function Page() {
    ...
    const [FavouriteSkinIds, setFavouriteSkinIds] = useState<Set<string>>(ReadFavouriteSkinIds);
    ...
    const ToggleFavouriteSkin = useCallback((Skin: SkinObject) => {
        setFavouriteSkinIds((CurrentFavouriteSkinIds) => {
            const NextFavouriteSkinIds = new Set(CurrentFavouriteSkinIds);
            const FavouriteKey = GetSkinFavouriteKey(Skin);

            if (NextFavouriteSkinIds.has(FavouriteKey)) {
                NextFavouriteSkinIds.delete(FavouriteKey);
            } else {
                NextFavouriteSkinIds.add(FavouriteKey);
            };

            WriteFavouriteSkinIds(NextFavouriteSkinIds);
            return NextFavouriteSkinIds;
        });
    }, []);
    ...
    const FilteredSkins = useMemo(() => {
        ...
        return Skins
        .filter((Skin) => {
            if (SkinFilter === "favourited") return FavouriteSkinIds.has(GetSkinFavouriteKey(Skin));
        ...
        .filter((Skin) => {
            ...
            const Searchable = [
                ...
                SkinTypes[Skin.type],
                IsSkinNew(Skin) ? "new" : "",
                FavouriteSkinIds.has(GetSkinFavouriteKey(Skin)) ? "favourited favorite favourite" : "",
    ...
    }, [FavouriteSkinIds, Search, SkinFilter, Skins]);
                    ...
                    <div className="flex-1 overflow-y-auto p-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                            {DisplaySkins.map((Skin) => (
                                <div key={Skin.id} className="relative cursor-pointer flex flex-col border border-zinc-800/75 bg-zinc-950/75 rounded-md p-4 transition hover:border-zinc-700/75 hover:bg-zinc-900/50" onClick={() => {Navigate(`/viewer?skin=${Skin.name}`)}}>
                                    <button aria-label={FavouriteSkinIds.has(GetSkinFavouriteKey(Skin)) ? "Remove from favourites" : "Add to favourites"} title={FavouriteSkinIds.has(GetSkinFavouriteKey(Skin)) ? "Remove from favourites" : "Add to favourites"} onClick={(Interaction) => {Interaction.stopPropagation(); ToggleFavouriteSkin(Skin)}} className={`absolute right-4 top-4 z-20 flex h-9 w-9 cursor-pointer items-center justify-center rounded-md border transition ${FavouriteSkinIds.has(GetSkinFavouriteKey(Skin)) ? "border-lime-500/30 bg-lime-500/10 text-lime-300" : "border-zinc-700 bg-zinc-900/90 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200"}`}>
                                        <Heart className={`h-4.5 w-4.5 ${FavouriteSkinIds.has(GetSkinFavouriteKey(Skin)) ? "fill-current" : ""}`}/>
                                    </button>

                                    <div className="flex items-center gap-4 overflow-hidden">
                                        <SkinPreviewImage src={Skin.image_url} alt={Skin.name}/>
